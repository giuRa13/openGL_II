...
{
    //convert fov in radians
    fovy = fovy * PI / 360.0f; // (half of fiel ov view Y)
    float t = tanf(fovy);
    float n = -near;
    float f = -far;
...
{
    //dot product = product of each component and sum them
...
vec3 normalize(vec3 v)
{
    ...
    // dot product of a vector with himself = squere of magnitude
    float magnitude = sqrtf(dot(v,v));

    //normalize = divide every component with his magnitude
...
{
    // produce a vector Ortogonal to the other 2 input vector
    vec3 w;
